...
.. c:type:: primme_params
   ...
   .. c:member:: primme_init initBasisMode
      ...
      Select how the search subspace basis are initialized up to |minRestartSize| vectors
      if not enough initial vectors are provided (see |initSize|):

      * ``primme_init_krylov``, with a Krylov subspace generated by the matrix problem and
        the last initial vector if given or a random vector otherwise.
   ...
   .. c:member:: primme_projection projectionParams.projection
      ...
      * ``primme_proj_ref``, refined extraction, compute :math:`||x_i||=1` so that
        minimizes :math:`||(A-\tau B)x_i||`; the eigenvalues are computes as the
   ...
   .. c:member:: double stats.maxConvTol
      ...
      Hold the maximum residual norm of the locked eigenvectors.
      The value is available during execution and at the end.
