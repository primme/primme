...
.. mat:function:: function [varargout] = primme_eigs(varargin)
   ...
   default values are indicated in brackets {}:
      ...
      * |aNorm|: the estimated 2-norm of A {estimate the norm internally}
      * ``tol``: convergence tolerance: ``NORM(A*X(:,i)-X(:,i)*D(i,i)) < tol*NORM(A)`` (see |eps|)
      ...
      * |robustShifts|: set to true may avoid stagnation or misconvergence 
   ...
   METHOD can be one of the next strings:
      ...
      * '|SUBSPACE_ITERATION|',       equivalent to GD(block,2*block)
