% TEST DRIVER for Model II from the genomics
% our goal: compute m desired number of singular vectors
% 1. Generate test matrix A
%       where A is simple model for human genotype data matrices in Matlab
%
%   Inputs: 
...
for w = 1:numel(nn) % iterate over different matrix dimensions A(n,d)
   ...
   for x = 1:size(mm,1) % iter over rank sizes of signal: m
        ...
        for y = 1:numel(zetas)
            ...
            r = 10; 
            nsignal = n*d; 
            A = makeGNmodel(n,d,r,nsignal,zeta,'',0); % no reading from disk. Generate A with the same seeds.
...
if (DO_SVDS), save('out_SVDS_II.mat','SVDS','-v7.3'); end;
if (DO_PSVDS), save('out_PSVDS_II.mat','PSVDS','-v7.3');end;
if (DO_RSVD), save('out_RSVD_II.mat','RSVD','-v7.3');end;
if (DO_FD), save('out_FD_II.mat','FD','-v7.3');end;
