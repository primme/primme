% TEST DRIVER for Model IV from the Frequent Directions paper
% our goal: compute m desired number of singular vectors
% 1. Generate test matrix A = SDU + N/zeta
%       where
...
%       zeta - scalar, for 1 <= zeta <= sqrt(d/m) signal recoverable, noise does not dominate
%
% 2. Run four methods
...
if (DO_FD)
  ...
  % FD(w,x,y,z).ells        % defined for each w,x,y problem
end
...
for w = 1:numel(nn) % iterate over different matrix dimensions A(n,d)
   ...
   for x = 1:size(mm,1) % iter over rank sizes of signal: m
        ...
        % Building the Fast Frequent Directions model matrix for various zetas
        zetas = [0.1 (1+sqrt(d/m))/2 sqrt(d/m) 10*sqrt(d/m) ];  % [full noise, borderline recoverable, recoverable, low rank]
        Zetas(count,:) = zetas;
        count = count + 1;
        ...
        for y = 1:numel(zetas)
            ...
            A = makeFDmodel(n,m,d,zeta,0,11,19); % no reading from disk. Generate A with the same seeds.
            ...
            if exist(svdinfo,'file')
                load(svdinfo,'u','s','v');
                DO_SVDS = 0; % Do NOT save SVDS struct. Use the one stored before when generating the svdinfo.
            else
                tic
...
if (DO_SVDS), save('out_SVDS_IV.mat','SVDS','-v7.3'); end;
if (DO_PSVDS), save('out_PSVDS_IV.mat','PSVDS','-v7.3');end;
if (DO_RSVD), save('out_RSVD_IV.mat','RSVD','-v7.3');end;
if (DO_FD), save('out_FD_IV.mat','FD','-v7.3');end;
