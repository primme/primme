% TEST DRIVER for Model III from the 3D Laplacian
% 1. Generate test matrix A = 3D laplacian
%       where
%       nodes = 15:15:150;
%
% 2. Run four methods
...
nn = [27e3 64e3];
% nn = [1e3]; % testing DELETE
nodes = [30 40]; % for 3D laplacian, be consistent with nn
% nodes = [10]; % testing DELETE
dd = nn;     % We fix dd = nn
...
for w = 1:numel(nn) % iterate over different matrix dimensions A(n,d)
   ...
   for x = 1:size(mm,1) % iter over rank sizes of signal: m
        ...
        % add a dummy variable for 3D laplacian
        zetas = [0];
        Zetas(count,:) = zetas;
        count = count + 1;
...
if (DO_SVDS), save('out_SVDS_III.mat','SVDS','-v7.3'); end;
if (DO_PSVDS), save('out_PSVDS_III.mat','PSVDS','-v7.3');end;
if (DO_RSVD), save('out_RSVD_III.mat','RSVD','-v7.3');end;
if (DO_FD), save('out_FD_III.mat','FD','-v7.3');end;
