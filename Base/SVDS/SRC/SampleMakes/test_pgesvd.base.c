...
extern @(rtype) verif_orthogonality(int m, int n, @(type) *U, int iu, int ju, int *descU);
extern @(rtype) verif_representativity(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                                     @(type) *U, int iu, int ju, int *descU,
                                                     @(type) *VT, int ivt, int jvt, int *descVT,
                                                     @(rtype) *S);
extern @(rtype) verif_repres_NV(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                              @(type) *VT, int ivt, int jvt, int *descVT,
                                              @(rtype) *S);
extern @(rtype) verif_repres_VN(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                              @(type) *U, int iu, int ju, int *descU,
                                              @(rtype) *S);
extern int driver_p@(pre)gesvd(char jobU, char jobVT, int m, int n, @(type) *A, int ia, int ja, int *descA,
			@(rtype) *S_NN, @(type) *U_NN, int iu, int ju, int *descU, @(type) *VT_NN, int ivt, int jvt, int *descVT,
		       	double *MPIelapsedNN);
...
int main(int argc, char **argv) {
	...
	@(type) *U_NN=NULL,  *U_VV=NULL,  *U_NV=NULL,  *U_VN=NULL;
	@(type) *VT_NN=NULL, *VT_VV=NULL, *VT_NV=NULL, *VT_VN=NULL;
	@(rtype) *S_NN=NULL,  *S_VV=NULL, *S_NV=NULL, *S_VN=NULL;
	@(rtype) *S_res_NN=NULL;
        @(rtype) orthU_VV, residF, orthVT_VV;
        @(rtype) orthU_VN, orthVT_NV;
	@(rtype)  residS_NN, eps;
	@(rtype)  res_repres_NV, res_repres_VN;
/**/
	int izero=0,ione=1;
	@(rtype) rtmone=@(rtmone);
/**/
...
/**/
	if (inputfromfile){
		...
		printf("\n");
		printf("--------------------------------------------------------------------------------------------------------------------\n");
				printf("                            Testing p@(pre)gsevd -- @(type) precision SVD ScaLAPACK routine                \n");
...
*
*/ 
		A = (@(type) *)calloc(mpA*nqA,sizeof(@(type))) ;
...
/**/		
		...
		for (i = 0; i < mpA; i++) {
			for (j = 0; j < nqA; j++) {
				@(pre)larnv_( &idist, iseed, &ione, &(A[k]) );
				k++;	
			}
		}
/*
...
/**/
		...
		nbtestcase++;	
		S_NN = (@(rtype) *)calloc(min_mn,sizeof(@(rtype))) ;
		if (S_NN==NULL){ printf("error of memory allocation S_NN on proc %dx%d\n",myrow,mycol); exit(0); }
		infoNN = driver_p@(pre)gesvd( 'N', 'N', m, n, A, 1, 1, descA,
			S_NN, U_NN, 1, 1, descU, VT_NN, 1, 1, descVT,
		       	&MPIelapsedNN);
		S_res_NN = (@(rtype) *)calloc(min_mn,sizeof(@(rtype))) ;
		if (S_res_NN==NULL){ printf("error of memory allocation S on proc %dx%d\n",myrow,mycol); exit(0); }
		@(rpre)copy_(&min_mn,S_VV,&ione,S_res_NN,&ione);
		@(rpre)axpy_ (&min_mn,&rtmone,S_NN,&ione,S_res_NN,&ione);
		residS_NN = @(rpre)nrm2_(&min_mn,S_res_NN,&ione) / @(rpre)nrm2_(&min_mn,S_VV,&ione);
		free(S_res_NN);
...
/**/		
@(rtype) verif_orthogonality(int m, int n, @(type) *U, int iu, int ju, int *descU){
	...
	int izero = 0, ione = 1;
	@(rtype) *wwork=NULL;
	@(type) tmone= @(tmone),  tpone= @(tpone),  tzero= @(tzero);
	@(rtype) orthU;
	...
	R = (@(type) *)calloc(mpR*nqR,sizeof(@(type))) ;
	...
	p@(pre)laset_( "F", &min_mn, &min_mn, &tzero, &tpone, R, &ione, &ione, descR );
	if (m>n)
		p@(pre)gemm_( @(TransConj), "N", &min_mn, &min_mn, &m, &tpone, U, &iu, &ju, descU, U, 
			&iu, &ju, descU, &tmone, R, &ione, &ione, descR );
	else
		p@(pre)gemm_( "N", @(TransConj), &min_mn, &min_mn, &n, &tpone, U, &iu, &ju, descU, U, 
			&iu, &ju, descU, &tmone, R, &ione, &ione, descR );
	orthU = p@(pre)lange_( "F", &min_mn, &min_mn, R, &ione, &ione, descR, wwork );
	orthU = orthU / ((@(rtype)) max_mn);
	free(R);
...
/**/		
@(rtype) verif_representativity(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                              @(type) *U, int iu, int ju, int *descU,
                                              @(type) *VT, int ivt, int jvt, int *descVT,
                                              @(rtype) *S){

	@(type) *Acpy=NULL, *Ucpy=NULL;
	int nprow, npcol, myrow, mycol;
	...
	Acpy = (@(type) *)calloc(mpA*nqA,sizeof(@(type))) ;
	if (Acpy==NULL){ printf("error of memory allocation Acpy on proc %dx%d\n",myrow,mycol); exit(0); }
      	p@(pre)lacpy_( "All", &m, &n, A, &ia, &ja, descA, Acpy, &ia, &ja, descA );
	...
	Ucpy = (@(type) *)calloc(mpU*nqU,sizeof(@(type))) ;
	if (Ucpy==NULL){ printf("error of memory allocation Ucpy on proc %dx%d\n",myrow,mycol); exit(0); }
      	p@(pre)lacpy_( "All", &m, &min_mn, U, &iu, &ju, descU, Ucpy, &iu, &ju, descU );

	AnormF = p@(pre)lange_( "F", &m, &n, A, &ia, &ja, descA, wwork);

	for (i=1;i<min_mn+1;i++){
		pcol = indxg2p_( &i, &(descU[5]), &izero, &izero, &npcol );
		localcol = indxg2l_( &i, &(descU[5]), &izero, &izero, &npcol );
		if( mycol==pcol )
			@(trpre)scal_( &mpA, &(S[i-1]), &(Ucpy[ ( localcol-1 )*descU[8] ]), &ione );
	}
	p@(pre)gemm_( "N", "N", &m, &n, &min_mn, &tpone, Ucpy, &iu, &ju, descU, VT, &ivt, &jvt, descVT,
	       		&tmone, Acpy, &ia, &ja, descA ); 
	residF = p@(pre)lange_( "F", &m, &n, Acpy, &ione, &ione, descA, wwork);
	residF = residF/AnormF/((@(rtype)) max_mn);
...
/**/		
@(rtype) verif_repres_NV(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                @(type) *VT, int ivt, int jvt, int *descVT,
                                @(rtype) *S){
	...
	@(rtype) verif_repres_NV, invStemp;
	...
	p@(pre)gemm_( "N", @(TransConj), &m, &min_mn, &n, &tpone, A, &ia, &ja, descA, VT, &ivt, &jvt, descVT,
	...
	for (i=1;i<min_mn+1;i++){
		...
		invStemp = 1/S[i-1];
		if( mycol==pcol )
			@(trpre)scal_( &mpA, &invStemp, &(Ucpy[ ( localcol-1 )*descUcpy[8] ]), &ione );
	}
...
/**/		
@(rtype) verif_repres_VN(int m, int n, @(type) *A, int ia, int ja, int *descA,
                                       @(type) *U, int iu, int ju, int *descU,
                                       @(rtype) *S){
	...
	int izero = 0, info;
	@(type) tpone= @(tpone),  tzero= @(tzero);
	@(rtype) verif_repres_VN, invStemp;
	...
	ivtcpy = 1; jvtcpy = 1;	
	VTcpy = (@(type) *)calloc(mpVT*nqVT,sizeof(@(type))) ;
	if (VTcpy==NULL){ printf("error of memory allocation VTcpy on proc %dx%d\n",myrow,mycol); exit(0); }

	p@(pre)gemm_( @(TransConj), "N", &min_mn, &n, &m, &tpone, U, &iu, &ju, descU, A, &ia, &ja, descA,
	...
	for (i=1;i<min_mn+1;i++){
		...
		invStemp = 1/S[i-1];
		if( myrow==prow )
			@(trpre)scal_( &nqA, &invStemp, &(VTcpy[localcol-1]), &mpVT );
	}
	...
	int driver_p@(pre)gesvd( char jobU, char jobVT, int m, int n, @(type) *A, int ia, int ja, int *descA,
		@(rtype) *S_NN, @(type) *U_NN, int iu, int ju, int *descU, @(type) *VT_NN, int ivt, int jvt, int *descVT,
	       	double *MPIelapsedNN){

	@(type) *Acpy=NULL, *work=NULL;
	int lwork;
...
/**/
	...
	if (Acpy==NULL){ printf("error of memory allocation Acpy on proc %dx%d\n",myrow,mycol); exit(0); }
      	...
      	p@(pre)lacpy_( "All", &m, &n, A, &ione, &ione, descA, Acpy, &ione, &ione, descA );


#ifdefarithm L_DEFREAL
	work = (@(type) *)calloc(1,sizeof(@(type))) ;
	if (work==NULL){ printf("error of memory allocation for work on proc %dx%d (1st time)\n",myrow,mycol); exit(0); }

	lwork=-1;

	p@(pre)gesvd_( &jobU, &jobVT, &m, &n, Acpy, &ione, &ione, descA,
	...
	work = (@(type) *)calloc(lwork,sizeof(@(type))) ;
...
/**/
	...
	rwork = (@(rtype) *)calloc(1,sizeof(@(rtype))) ;
	if (rwork==NULL){ printf("error of memory allocation for work on proc %dx%d \n",myrow,mycol); exit(0); }

	lwork=-1;

	p@(pre)gesvd_( &jobU, &jobVT, &m, &n, Acpy, &ione, &ione, descA,
	       	S_NN, U_NN, &ione, &ione, descU, VT_NN, &ione, &ione, descVT,
	       	work, &lwork, rwork, &infoNN);
	...
	work = (@(type) *)calloc(lwork,sizeof(@(type))) ;
	if (work==NULL){ printf("error of memory allocation for work on proc %dx%d \n",myrow,mycol); exit(0); }
	rwork = (@(rtype) *)calloc(lrwork,sizeof(@(rtype))) ;
	if (rwork==NULL){ printf("error of memory allocation for work on proc %dx%d \n",myrow,mycol); exit(0); }
/**/		
	MPIt1 = MPI_Wtime();
/**/
