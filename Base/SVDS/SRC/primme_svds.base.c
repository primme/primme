...
static @(type)* copy_last_params_from_svds(primme_svds_params *primme_svds, int stage,
   ...
   *allocatedTargetShifts = 0;
   ...
   case primme_svds_op_augmented:
      ...
      Num_copy_@(pre)primme(primme->nLocal*n, svecs, 1, aux, 1);
   ...
   if (method == primme_svds_op_augmented && primme->initSize <= 0) {
      ...
      if (primme_svds->m >= primme_svds->n) {
         Num_larnv_@(pre)primme(2, primme->iseed, primme_svds->mLocal,
         ...
         Num_larnv_@(pre)primme(2, primme->iseed, primme_svds->nLocal, svecs);
         primme_svds->matrixMatvec(svecs, &primme_svds->nLocal,
               &svecs[primme_svds->nLocal], &primme_svds->mLocal, &ONE,
               &NOTRANS, primme_svds);
      }
      norms2_[0] = REAL_PART(Num_dot_@(pre)primme(primme_svds->nLocal, svecs, 1,
               svecs, 1));
      norms2_[1] = REAL_PART(Num_dot_@(pre)primme(primme_svds->mLocal,
               &svecs[primme_svds->nLocal], 1, &svecs[primme_svds->nLocal], 1));
      globalSum_dprimme_svds(norms2_, norms2, 2, primme_svds);
      Num_scal_@(pre)primme(primme_svds->nLocal, 1.0/sqrt(norms2[0]), svecs, 1);
      Num_scal_@(pre)primme(primme_svds->mLocal, 1.0/sqrt(norms2[1]),
...
static void copy_last_params_to_svds(primme_svds_params *primme_svds, int stage,
   ...
   case primme_svds_op_augmented:
      ...
      norms2 = norms2_ + 2*n;
      for (i=0; i<n; i++) {
         norms2_[i] = REAL_PART(Num_dot_@(pre)primme(primme_svds->mLocal,
            &svecs[primme_svds->mLocal*i], 1, &svecs[primme_svds->mLocal*i],
            1));
      }
      for (i=0; i<n; i++) {
         norms2_[n+i] = REAL_PART(Num_dot_@(pre)primme(primme_svds->nLocal,
               &svecs[primme_svds->mLocal*n+primme_svds->nLocal*i], 1,
               &svecs[primme_svds->mLocal*n+primme_svds->nLocal*i], 1));
      }
      globalSum_dprimme_svds(norms2_, norms2, 2*n, primme_svds);
      for (i=0; i<n; i++) {
         Num_scal_@(pre)primme(primme_svds->mLocal, 1.0/sqrt(norms2[i]),
               &svecs[primme_svds->mLocal*i], 1);
      }
      for (i=0; i<n; i++) {
         Num_scal_@(pre)primme(primme_svds->nLocal, 1.0/sqrt(norms2[n+i]),
               &svecs[primme_svds->mLocal*n+primme_svds->nLocal*i], 1);
      }
      free(norms2_);
      break;
...
static void Num_scalInv_@(pre)matrix(@(type) *x, int m, int n, int ldx,
   ...
   for (i=0; i<n; i++) {
      if (factors[i] > 0.0 && 1.0L/factors[i] < HUGE_VAL) {
         ...
         norm0 = REAL_PART(Num_dot_@(pre)primme(m, &x[i*ldx], 1, &x[i*ldx], 1));
         globalSum_dprimme_svds(&norm0, &norm, 1, primme_svds);
         factor = sqrt(norm);
      }
      Num_scal_@(pre)primme(m, 1.0/factor, &x[i*ldx], 1);
   }
}
