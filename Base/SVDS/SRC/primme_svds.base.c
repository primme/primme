...
static @(type)* copy_last_params_from_svds(primme_svds_params *primme_svds, int stage,
   ...
   *allocatedTargetShifts = 0;
   ...
   if (method == primme_svds_op_augmented && primme->initSize <= 0) {
      ...
      ztmp = Num_dot_@(pre)primme(primme_svds->nLocal, svecs, 1, svecs, 1);
      dtmp[0] = *(double*)&ztmp;
      ztmp = Num_dot_@(pre)primme(primme_svds->mLocal,
      ...
      *(double*)&ztmp = 1.0/sqrt(dtmp0[0]);
      Num_scal_@(pre)primme(primme_svds->nLocal, ztmp, svecs, 1);
