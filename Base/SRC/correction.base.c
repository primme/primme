...
int solve_correction_@(pre)primme(SCALAR *V, SCALAR *W, SCALAR *evecs, 
      ...
      if ( primme->correctionParams.projectors.RightX &&
         ...
         if ( primme->correctionParams.projectors.RightX ) {   
            ...
            for (blockIndex = 0; blockIndex < blockSize; blockIndex++) {
               ...
               Num_axpy_@(pre)primme(primme->nLocal, -approxOlsenEps[blockIndex],
   ...
   /* ------------------------------------------------------------ */
      ...
      for (blockIndex = 0; blockIndex < blockSize; blockIndex++) {
         ...
         CHKERR(inner_solve_@(pre)primme(x, r, &blockNorms[blockIndex], evecs, 
         ...
         Num_copy_@(pre)primme(primme->nLocal, sol, 1, 
...
static int apply_preconditioner_block(SCALAR *v, SCALAR *result, 
   ...
   if (primme->correctionParams.precondition) {
      ...
      Num_copy_@(pre)primme(primme->nLocal*blockSize, v, 1, result, 1);
   }
...
static int Olsen_preconditioner_block(SCALAR *r, SCALAR *x,
   ...
   CHKERR(globalSum_@(pre)primme(xKinvx_local, xKinvx, 2*blockSize, primme),
         -1);
   ...
   if (primme->correctionParams.projectors.LeftQ) {
         ...
         *Lprojector = evecs;
         if (primme->correctionParams.projectors.LeftX) {
            Num_copy_@(pre)primme(n, x, 1, &evecs[sizeEvecs*n], 1);
   ...
   if (primme->correctionParams.projectors.RightX) {
      ...
      if (primme->correctionParams.precondition   &&
         ...
         *RprojectorX  = Kinvx;
         ...
         CHKERR(globalSum_@(pre)primme(&xKinvx_local, xKinvx, 1, primme), -1);
      }      
      else {
         *RprojectorX = x;
         *xKinvx = 1.0;
