...
int main_iter_@(pre)primme(double *evals, int *perm, SCALAR *evecs, 
   ...
   if (primme->n == 1) {
      evecs[0] = 1.0;
      CHKERR(matrixMatvec_@(pre)primme(&evecs[0], primme->nLocal, primme->nLocal,
   ...
   CHKERR(init_basis_@(pre)primme(V, primme->nLocal, primme->nLocal, W,
   ...
   while (!converged &&
      ...
      targetShiftIndex = 0;
      if (Q) CHKERR(update_Q_@(pre)primme(V, primme->nLocal, primme->nLocal, W,
      ...
      while (numConverged < primme->numEvals &&
         ...
         while (basisSize < primme->maxBasisSize &&
            ...
            if (blockSize == 0) {
               ...
               CHKERR(solve_correction_@(pre)primme(V, W, evecs, evecsHat, UDU, 
            ...
            CHKERR(ortho_@(pre)primme(V, primme->nLocal, NULL, 0, basisSize, 
            ...
            CHKERR(matrixMatvec_@(pre)primme(V, primme->nLocal, primme->nLocal,
                     W, primme->nLocal, basisSize, blockSize, primme), -1);

            if (Q) CHKERR(update_Q_@(pre)primme(V, primme->nLocal,
            ...
            if (basisSize+blockSize >= primme->maxBasisSize) {
               CHKERR(retain_previous_coefficients_@(pre)primme(hVecs,
         ...
         if (targetShiftIndex >= 0) {
            ...
            permute_vecs_@(pre)primme(hVecs, basisSize, basisSize, basisSize, iwork, rwork, iwork+basisSize);
      ...
      if (primme->locking) {
         ...
         if (restartLimitReached || converged || wholeSpace) {
            ...
            /* ------------------------------------------------------------ */
            CHKERR(ortho_@(pre)primme(V, primme->nLocal, NULL, 0, 0,
...
int prepare_candidates_@(pre)primme(SCALAR *V, SCALAR *W, PRIMME_INT nLocal,
   ...
   if (V == NULL) {
      ...
      CHKERR(check_convergence_@(pre)primme(NULL, nLocal, 0, NULL, 0, NULL,
      ...
      lrw = max(lrw,
            (size_t)Num_update_VWXR_@(pre)primme(NULL, NULL, nLocal, basisSize,
               ...
               NULL, 0, 0,
               NULL, 0, primme));
      CHKERR(prepare_vecs_@(pre)primme(basisSize, 0, maxBlockSize, NULL, 0,
   ...
   *recentlyConverged = 0;
   while (1) {
      ...
      CHKERR(check_convergence_@(pre)primme(X?&X[(*blockSize)*ldV]:NULL, nLocal,
      ...
      for (blki=*blockSize, i=0; i < blockNormsSize && *blockSize < maxBlockSize; i++, blki++) {
         ...
         if ((primme->target == primme_closest_leq
            ...
            *smallestResNorm = min(*smallestResNorm, blockNorms[blki]);
            ...
            if (X) Num_copy_matrix_@(pre)primme(&X[blki*ldV], nLocal, 1, ldV,
                  &X[(*blockSize)*ldV], ldV);
            if (R) Num_copy_matrix_@(pre)primme(&R[blki*ldV], nLocal, 1, ldV,
                  &R[(*blockSize)*ldV], ldV);
            (*blockSize)++;
         }
      ...
      CHKERR(Num_update_VWXR_@(pre)primme(V, W, nLocal, basisSize, ldV,
...
static int verify_norms(SCALAR *V, SCALAR *W, double *hVals, int basisSize,
   ...
   CHKERR(check_convergence_@(pre)primme(V, primme->nLocal, primme->nLocal,
