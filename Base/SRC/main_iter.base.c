...
int main_iter_@(pre)primme(double *evals, int *perm, @(type) *evecs, 
   ...
   @(type) *WTW;             /* Upper triangular portion of W'*W            */ 
   @(type) *wtwChol;         /* Copy of WTW for sloving generalized eigenproblem */
   @(type) *Q;               /* (A-RefShifts(1)I)*V = W-RefShifts(1)*V = QR */
   @(type) *R;               /* (A-RefShifts(1)I)*V = W-RefShifts(1)*V = QR */                            
   ...
   while (!converged &&
      ...
      ret = solve_H_@(pre)primme(H, hVecs, Q, R, hVals, basisSize, primme->maxBasisSize,
      ...
      while (numConverged < primme->numEvals &&
         ...
         while (basisSize < primme->maxBasisSize 
            ...
            ret = ortho_@(pre)primme(V, NULL, primme->nLocal, basisSize, 
        ...
        if (primme->qr_need == 0 && Q!= NULL && R!= NULL) 
         ...
         if (primme->locking) {
            ret = lock_vectors_@(pre)primme(tol, &primme->currentEstimates.Anormest, &maxConvTol,
      ...
      if (primme->locking) {
         ...
         if (restartLimitReached || converged) {
            ...
            ret = ortho_@(pre)primme(V, NULL, primme->nLocal,0, basisSize-1, evecs, 
