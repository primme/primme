...
int init_basis_@(pre)primme(@(type) *V, @(type) *W, @(type) *evecs, 
   ...
   if (*basisSize < primme->maxBlockSize) {
      Num_larnv_@(pre)primme(2, primme->iseed,primme->nLocal*(primme->maxBlockSize-*basisSize),&V[primme->nLocal*(*basisSize)]);
      ret = ortho_@(pre)primme(V, NULL, primme->nLocal, *basisSize, primme->maxBlockSize, evecs, 
      ...
      update_W_@(pre)primme(V, W, *basisSize, primme->maxBlockSize, primme);
