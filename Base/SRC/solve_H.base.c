...
static int solve_H_Harm_@(pre)primme(SCALAR *H, int ldH, SCALAR *QtV, int ldQtV,
...
static int solve_H_Ref_@(pre)primme(SCALAR *H, int ldH, SCALAR *hVecs,
...
int solve_H_@(pre)primme(SCALAR *H, int basisSize, int ldH, SCALAR *R, int ldR,
   ...
   case primme_proj_RR:
      CHKERR(solve_H_RR_@(pre)primme(H, ldH, hVecs, ldhVecs, hVals, basisSize,
               numConverged, lrwork, rwork, liwork, iwork, primme), -1);
      break;

   case primme_proj_harmonic:
      CHKERR(solve_H_Harm_@(pre)primme(H, ldH, QtV, ldQtV, R, ldR, hVecs,
               ldhVecs, hU, ldhU, hVals, basisSize, numConverged, machEps,
               lrwork, rwork, liwork, iwork, primme), -1);
      break;

   case primme_proj_refined:
      CHKERR(solve_H_Ref_@(pre)primme(H, ldH, hVecs, ldhVecs, hU, ldhU, hSVals, 
...
static int solve_H_Harm_@(pre)primme(SCALAR *H, int ldH, SCALAR *QtV, int ldQtV,
   ...
   /* Return memory requirements */
   if (QtV == NULL) {
      CHKERR(solve_H_RR_@(pre)primme(QtV, ldQtV, hVecs, ldhVecs, hVals, basisSize,
   ...
   ret = solve_H_RR_@(pre)primme(hVecs, ldhVecs, hVecs, ldhVecs, hVals,
         basisSize, 0, lrwork, rwork, liwork, iwork, primme);
   primme->targetShifts = oldTargetShifts;
   primme->target = oldTarget;
   CHKERRM(ret, -1, "Error calling solve_H_RR_@(pre)primme\n");
   ...
   Num_trsm_@(pre)primme("L", "U", "N", "N", basisSize, basisSize, 1.0, R, ldR,
         hVecs, ldhVecs);
   CHKERR(ortho_@(pre)primme(hVecs, ldhVecs, NULL, 0, 0, basisSize-1, NULL, 0, 0,
         basisSize, primme->iseed, machEps, rwork, lrwork, primme), -1);
 
   /* Compute Rayleigh quotient lambda_i = x_i'*H*x_i */

   Num_hemm_@(pre)primme("L", "U", basisSize, basisSize, 1.0, H,
...
int prepare_vecs_@(pre)primme(int basisSize, int i0, int blockSize,
   ...
   if (H == NULL) {
      size_t rworkSize0=0;
      CHKERR(compute_submatrix_@(pre)primme(NULL, basisSize, 0, NULL,
               basisSize, 0, NULL, 0, NULL, &rworkSize0), -1);
      CHKERR(solve_H_RR_@(pre)primme(NULL, 0, NULL, 0, NULL, basisSize, 0,
   ...
   for (candidates=0, i=min(*arbitraryVecs,basisSize), j=i0;
      ...
      if (i-j > 1 && (someCandidate || RRForAll)) {
         ...
         /* Compute and sort eigendecomposition aH*ahVecs = ahVecs*diag(hVals(j:i-1)) */
         CHKERR(solve_H_RR_@(pre)primme(aH, aBasisSize, ahVecs, ldhVecsRot,
