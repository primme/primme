...
int @(pre)primme(double *evals, @(type) *evecs, double *resNorms, 
   ...
   permute_vecs_@(pre)(&evecs[primme->numOrthoConst], primme->nLocal,
...
static int allocate_workspace(primme_params *primme, int allocate) {
   ...
   solveHSize = solve_H_@(pre)primme(NULL, primme->maxBasisSize, 0, NULL, 0, NULL, 0, NULL,
   ...
   if (primme->locking) {
      restartSize = restart_locking_@(pre)primme(&primme->maxBasisSize, NULL,
      ...
      restartSize = restart_@(pre)primme(&primme->maxBasisSize, NULL, NULL,
   ...
   restartSize += after_restart_@(pre)primme(NULL, 0, NULL, 0, NULL, 0, NULL, 0, 0,
         NULL, 0, NULL, 0, 0,
   ...
   mainSize = max(
         ...
         prepare_candidates_@(pre)(NULL, NULL, primme->nLocal, primme->maxBasisSize,
