...
int restart_locking_@(pre)primme(int *restartSize, SCALAR *V, SCALAR *W, 
   ...
   if (V == NULL) {
      ...
      *rworkSize = max(*rworkSize, (size_t)
            Num_compute_residual_columns_@(pre)primme(nLocal, NULL, NULL,
               basisSize, NULL, 0, NULL, 0, NULL, primme->maxBlockSize, 0, 0,
               NULL, 0, NULL, primme->maxBlockSize, NULL, 0, NULL, 0, NULL, 0));
      CHKERR(Num_reset_update_VWXR_@(pre)primme(NULL, NULL, nLocal, basisSize,
   ...
   *R = &W[*restartSize*ldV];
   ...
   CHKERR(check_convergence_@(pre)primme(&V[ldV*left],
