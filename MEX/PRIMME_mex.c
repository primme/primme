 ...
 *
 *[1] 'flag' is to mark the input matrix is real or complex. 
 *[2] 'dim' is the dimension of large and sparse symmetric matrix.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    mwSize i, j;
    mwSize ndim;
    mxArray *tmp; /* use this mxArray pointer to store each field value pointer of opts*/
	...
	if (nrhs >= 2) {
	   if(!mxIsEmpty(prhs[1])) {
	      primme.n = (int)mxGetScalar(prhs[1]); /*get the dimension of the constructed matrix*/
	...
	if (nrhs >= 3) {
	   if((int)mxGetScalar(prhs[2])<= primme.n)
	      primme.numEvals = (int)mxGetScalar(prhs[2]); /*get the output number of eigenvalues*/
	   else
	   ...
	   if (!mxIsChar(prhs[3]) || (mxGetM(prhs[3]) != 1 ))
	      mexErrMsgTxt("The target argument must be a string");
	       ...
	       if(!strcmp(read_target_string,"SA")) {
              primme.target = 0; /*get the target method to output the eigenvalues*/
	...
	if (nrhs >= 5) {
	   if((int)mxGetScalar(prhs[4]) >=0 && (int)mxGetScalar(prhs[4])<=14)
	       method = (int)mxGetScalar(prhs[4]); /*get the sovler method the user chooses*/ 
	   else
 ...
 * }*/
	...
	if (nrhs == 6) {
	   if (!mxIsStruct(prhs[5]))
	       mexErrMsgTxt("The opts must be a structure");
	         ...
	         if(tmp != NULL) {  
	            outputfilename[0] = mxArrayToString(tmp);
                primme.outputFile = fopen(outputfilename[0], "a+");
                mexPrintf("primme.outputFileName[0] is:%s\n",outputfilename[0]);
                ndim = mxGetN(prhs[5]); /*get the colomn dimension of struct*/
	         }
    ...
    mexPrintf("finish reading input paramters...\n");
	mexPrintf("ready to judge if the matrix A is real or complex\n");

	if (!mxIsComplex(prhs[0]))
	{
	    mexPrintf("The matrix A is real\n");
        /* set MatrixMatvec function to do Matrix-vector multiplication job*/
	    primme.matrixMatvec = MatrixMatvec_d; 
        /* set Precontioner function to do preconditioning job*/
	    primme.applyPreconditioner = Preconditioner_d; 
        /*set the method for sloving the problem. */   
/*	    primme_set_method(method, &primme);*/ 

	    double *evecs = (double *)EVecs;
	    int num;
	    num = max(primme.numEvals,primme.initSize); 
        /*When locking is 0, num gets larger one between two parameters*/
	    num = num + primme.numOrthoConst; 
        /*get how large number needed to allocate evecs memory */
...
/*           primme_display_params(primme);*/
        ...
        primme_Free(&primme);
        for (i=0;i<2;i++){
            if(outputfilename[i] != NULL){
                mxFree(outputfilename[i]);
            }
        }      
    }
	else
    {
	    ...
	    primme.matrixMatvec = MatrixMatvec_z; /* set MatrixMatvec function to do Matrix-vector multiplication job*/
	    primme.applyPreconditioner = Preconditioner_z; /* set Precontioner function to do preconditioning job*/
	    primme_set_method(method, &primme); /*set the method for sloving the problem. */   
	    ...
	    int ret = zprimme(NULL,NULL,NULL,&primme); /*set the max primme.realWorkSize and primme.intWorkSize*/
	    mexPrintf("ret value for reporting memory is %d\n", ret);
	    mexPrintf("The intworksize is %d\n", primme.intWorkSize);
            mexPrintf("The realworksize is %d\n", primme.realWorkSize);
...
/******************************************************************************
 *
 * Matvec function. Calls the necessary function call in MATLAB.
 *
******************************************************************************/

/* Pass blocked vector x into matvec.m function or matvec functon user defined, 
 * in which do matrix-vector multiplicaiton on A and x, and then return the 
 * correct blocked vector to y. If A is a matrix, y = A*x; If A is a function
 * matrix, then y = afun(x) where performs function matrix A * x. */
...
/* Pass blocked vector x into the preconditioner functon that user defined, 
 * in which do matrix-vector multiplicaiton on P and x, and then return the 
 * correct blocked vector to y. If P is a matrix, y = P\x; If P is a function
 * matrix, then y = afun(x) where performs function matrix P\x. */ 
