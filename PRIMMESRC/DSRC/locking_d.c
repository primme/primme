...
int restart_locking_dprimme(int *restartSize, double *V, double *W,
   ...
   if (V == NULL) {
      ...
      return max(max(max(max(
            /* for permute_vecs */
            basisSize,
            Num_compact_res_i_dprimme(nLocal, NULL, NULL, basisSize, 0, NULL, 0,
   ...
   *indexOfPreviousVecs = *restartSize;
   ...
   permute_vecs_d(hVecs, basisSize, basisSize, ldhVecs, hVecsperm, rwork, iwork);
   ...
   *restartSize += numPacked + *numPrevRetained;
   ...
   Num_compact_res_i_dprimme(nLocal, &hVals[left], &V[left*ldV], failed, left, ifailed, 
         ldV, &W[left*ldV], ldV,
