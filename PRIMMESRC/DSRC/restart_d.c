...
int restart_dprimme(int *restartSize, double *V, double *W, int nLocal,
   ...
   *numConverged = min(*numConverged, primme->numEvals);
   ...
   permute_vecs_d(hVecs, basisSize, basisSize, ldhVecs, hVecsperm, rwork,
         iwork);
   ...
   *restartSize += *numPrevRetained;
   ...
   if (evecsHat) {
      ...
      for (i=0; i < *numConverged && hVecsperm[i] < *numConvergedStored; i++) {
         Num_copy_matrix_dprimme(&evecs[(hVecsperm[i]+primme->numOrthoConst)*nLocal],
               nLocal, 1, nLocal,
               &evecs[(newNumConvergedStored+primme->numOrthoConst)*nLocal],
               nLocal);
         Num_copy_matrix_dprimme(&evecsHat[(hVecsperm[i]+primme->numOrthoConst)*ldevecsHat],
               nLocal, 1, ldevecsHat,
