...
static int restart_soft_locking_dprimme(int *restartSize, SCALAR *V,
...
static int restart_projection_dprimme(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
...
static int dtr_dprimme(int numLocked, SCALAR *hVecs, double *hVals, int *flags, 
  int basisSize, int numFree, int *iev, SCALAR *rwork, primme_params *primme);

static int ortho_coefficient_vectors_dprimme(SCALAR *hVecs, int basisSize,
...
int restart_dprimme(SCALAR *V, SCALAR *W, PRIMME_INT nLocal, int basisSize,
   ...
   if (V == NULL) {
      iworkSize0 = 0;
      CHKERR(ortho_coefficient_vectors_dprimme(NULL, basisSize, 0,
            basisSize, NULL, 0, NULL, 0, numPrevRetained, 0.0, NULL, rworkSize,
            primme), -1);

      if (primme->locking) {
         CHKERR(restart_locking_dprimme(&basisSize, NULL, NULL, nLocal,
         ...
         CHKERR(restart_soft_locking_dprimme(&basisSize, NULL, NULL,
      ...
      CHKERR(restart_projection_dprimme(NULL, 0, NULL, 0, NULL, 0, NULL, 0, 0,
   ...
   *numPrevRetained = max(0, min(min(
   ...
   CHKERR(ortho_coefficient_vectors_dprimme(hVecs, basisSize, ldhVecs,
   ...
   if (!primme->locking) {
      SCALAR *X, *Res;
      CHKERR(restart_soft_locking_dprimme(&restartSize, V, W, nLocal,
      ...
      SCALAR *X, *Res;
      CHKERR(restart_locking_dprimme(&restartSize, V, W, nLocal, basisSize,
   ...
   *reset = 0;
   ...
   CHKERR(restart_projection_dprimme(V, ldV, W, ldV, H, ldH, Q, ldQ,
   ...
   if (*numConverged >= primme->numEvals && !primme->locking) {
      permute_vecs_dprimme(V, nLocal, restartSize, ldV, hVecsPerm, rwork,
            iwork0);
   }
...
static int restart_soft_locking_dprimme(int *restartSize, SCALAR *V,
   ...
   if (V == NULL) {
      ...
      *rworkSize = max(*rworkSize, (size_t)basisSize); /* permute_vecs for hVecs */
      CHKERR(Num_reset_update_VWXR_dprimme(NULL, NULL, nLocal, basisSize, 0, &t,
            *restartSize, 0, NULL,
            &t, 0, *restartSize, 0,
   ...
   *R = &W[*restartSize*ldV];
   ...
   CHKERR(Num_reset_update_VWXR_dprimme(V, W, nLocal, basisSize, ldV,
   ...
   if (!wholeSpace) {
      ...
      CHKERR(check_convergence_dprimme(V, nLocal, ldV, NULL, 0, NULL, 0, 0,
      ...
      *numConverged = 0;
      ...
      permute_vecs_dprimme(hVals, 1, *restartSize, 1, hVecsPerm, (double*)rwork,
            iwork);
      permute_vecs_dprimme(hVecs, basisSize, *restartSize, ldhVecs,
...
int Num_reset_update_VWXR_dprimme(SCALAR *V, SCALAR *W, PRIMME_INT mV,
   ...
   if (V == NULL) {
      *lrwork = max(*lrwork,
            (size_t)Num_update_VWXR_dprimme(
   ...
   /* Quick exit */
   if (reset == 0) {
      CHKERR(Num_update_VWXR_dprimme(
               V, W, mV, nV, ldV, h, nh, ldh, hVals,
               X0, nX0b, nX0e, ldX0,
               X1, nX1b, nX1e, ldX1,
   ...
   if (reset > 1) {
      CHKERR(ortho_dprimme(evecs, ldevecs, NULL, 0, evecsSize, 
               evecsSize+nX2e-nX2b-1, NULL, 0, 0, mV, primme->iseed, 
               machEps, rwork, lrwork, primme), -1);
      CHKERR(ortho_dprimme(X0, ldX0, NULL, 0, 0, nX2b-nX0b-1, evecs,
      ...
      CHKERR(ortho_dprimme(X0, ldX0, NULL, 0, nX2e-nX0b, nX0e-nX0b-1,
   ...
   CHKERR(matrixMatvec_dprimme(X0, mV, ldX0, Wo, ldWo, 0, nWoe-nWob,
            primme), -1);
   ...
   for (j=nRb; j<nRe; j++) {
      ...
      if (Rnorms) {
         Rnorms[j-nRb] = REAL_PART(Num_dot_dprimme(mV, &R[ldR*(j-nRb)], 1,
                  &R[ldR*(j-nRb)], 1));
      }
   }
   ...
   if (rnorms) for (j=nrb; j<nre; j++) {
      ...
      for (i=0; i < mV; i+=m, m=min(m,mV-i)) {
         ...
         rnorms[j-nrb] +=
            REAL_PART(Num_dot_dprimme(m, rwork, 1, rwork, 1));
      }
   }
...
static int restart_projection_dprimme(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
   ...
   if (evecsHat) {
      ...
      /* Return memory requirement */
      if (H == NULL) {
         CHKERR(update_projection_dprimme(NULL, 0, NULL, 0, NULL, 0, nLocal,
                  *evecsSize, basisSize, NULL, rworkSize, 1/*symmetric*/,
                  primme), -1);
         CHKERR(UDUDecompose_dprimme(NULL, 0, NULL, 0, NULL, *evecsSize,
      ...
      CHKERR(update_projection_dprimme(evecs, primme->nLocal, evecsHat,
...
static int restart_RR(SCALAR *H, int ldH, SCALAR *hVecs, int ldhVecs,
   ...
   if (H == NULL) {
      CHKERR(compute_submatrix_dprimme(NULL, numPrevRetained, 0, NULL,
               basisSize, 0, NULL, 0, NULL, rworkSize), -1);
      CHKERR(solve_H_dprimme(NULL, numPrevRetained, 0, NULL, 0, NULL, 0,
   ...
   CHKERR(compute_submatrix_dprimme(&hVecs[ldhVecs*indexOfPreviousVecs],
...
static int restart_refined(SCALAR *V, PRIMME_INT ldV, SCALAR *W, PRIMME_INT ldW,
   ...
   if (V == NULL) {
      CHKERR(compute_submatrix_dprimme(NULL, basisSize, 0, NULL, basisSize,
               0, NULL, 0, NULL, rworkSize), -1);
      CHKERR(update_Q_dprimme(NULL, nLocal, 0, NULL, 0, NULL, 0, NULL, 0,
               0.0, 0, basisSize, NULL, rworkSize, 0.0, primme), -1);
      CHKERR(update_projection_dprimme(NULL, 0, NULL, 0, NULL, 0, nLocal, 0, basisSize,
      ...
      *rworkSize = max(*rworkSize,
            (size_t)Num_update_VWXR_dprimme(NULL, NULL, nLocal, basisSize,
               ...
               NULL, 0, 0,
               NULL, 0, primme));
      CHKERR(solve_H_dprimme(NULL, basisSize, 0, NULL, 0, NULL, 0, NULL, 0,
   ...
   if (H) compute_submatrix_dprimme(hVecs, restartSize, ldhVecs, H,
   ...
   if (*targetShiftIndex < 0 || primme->targetShifts[*targetShiftIndex]
      ...
      *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
      ...
      CHKERR(update_Q_dprimme(V, nLocal, ldV, W, ldW, Q, ldQ, R, ldR,
               primme->targetShifts[*targetShiftIndex], 0,
               restartSize, rwork, rworkSize, machEps, primme), -1);

      CHKERR(solve_H_dprimme(H, restartSize, ldH, R, ldR, NULL, 0, hU,
   ...
   Num_gemm_dprimme("N", "N", basisSize, numPrevRetained, basisSize, 1.0,
   ...
   Num_zero_matrix_dprimme(hVecsRot0, mhVecsRot0, nRegular, mhVecsRot0);
   Num_copy_matrix_columns_dprimme(hVecsRot, *numArbitraryVecs,
   ...
   /* R = zeros() */
   Num_zero_matrix_dprimme(R, primme->maxBasisSize, primme->maxBasisSize,
         ldR);
   
   /* [hVecsRot0, R] = ortho(hVecsRot0) */
   CHKERR(ortho_dprimme(hVecsRot0, mhVecsRot0, R, ldR, 0, nRegular-1, NULL,
         0, 0, mhVecsRot0, primme->iseed, machEps, rwork0, &rworkSize0, NULL),
         -1);

   /* hU = hU * hVecsRot0 */
   Num_gemm_dprimme("N", "N", basisSize, nRegular, mhVecsRot0, 1.0, hU,
         ldhU, hVecsRot0, mhVecsRot0, 0.0, rwork0, basisSize);
   Num_copy_matrix_dprimme(rwork0, basisSize, nRegular, basisSize, hU,
         basisSize);

   /* hU = [hU RPrevhVecs] */
   Num_copy_matrix_dprimme(RPrevhVecs, basisSize, numPrevRetained,
         basisSize, &hU[basisSize*nRegular], basisSize);

   /* [hU, R] = ortho(hU, nRegular:nRegular+numPrevRetained-1) */
   CHKERR(ortho_dprimme(hU, basisSize, R, ldR, nRegular,
   ...
   CHKERR(Num_update_VWXR_dprimme(Q, NULL, nLocal, basisSize, ldQ, hU,
            restartSize,
            basisSize, NULL,
            Q, 0, restartSize, ldQ,
            NULL, 0, 0, 0,
   ...
   CHKERR(solve_H_dprimme(H, restartSize, ldH, R, ldR, NULL, 0, hU, newldhU,
...
static int restart_harmonic(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
   ...
   CHKERR(compute_submatrix_dprimme(hVecs, restartSize, ldhVecs, H,
   ...
   *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
   ...
   CHKERR(update_projection_dprimme(Q, ldQ, V, ldV, QtV, ldQtV, nLocal, 0,
   ...
   CHKERR(solve_H_dprimme(H, restartSize, ldH, R, ldR, QtV, ldQtV, hU,
            newldhU, hVecs, newldhVecs, hVals, hSVals, numConverged, machEps,
            rworkSize, rwork, iworkSize, iwork, primme), -1);
  ...
  int basisSize, int numFree, int *iev, SCALAR *rwork, primme_params *primme)
{
   ...
   Num_copy_dprimme(i*basisSize, &hVecs[basisSize*lOpt], 1, rwork, 1);
   Num_copy_dprimme(rOpt*basisSize, &hVecs[basisSize*(basisSize-rOpt)], 1,
      &hVecs[basisSize*lOpt], 1);
   Num_copy_dprimme(i*basisSize, rwork, 1, &hVecs[basisSize*restartSize], 1);
...
static int ortho_coefficient_vectors_dprimme(SCALAR *hVecs, int basisSize,
   ...
   if (primme->projectionParams.projection == primme_proj_harmonic) {
      ...
      CHKERR(ortho_dprimme(hVecs?&hVecs[ldhVecs*indexOfPreviousVecs]:NULL,
      ...
      if (hVecs) Num_trsm_dprimme("L", "U", "N", "N", basisSize,
   ...
   CHKERR(ortho_dprimme(hVecs, ldhVecs, !outR ? NULL :
...
int retain_previous_coefficients_dprimme(SCALAR *hVecs, int ldhVecs,
   ...
   *numPrevRetained = i;
   ...
   if (primme->projectionParams.projection == primme_proj_RR
      ...
      Num_zero_matrix_dprimme(previousHVecs+basisSize, mprevious-basisSize,
