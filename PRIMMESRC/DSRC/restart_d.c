...
int restart_dprimme(double *V, double *W, int nLocal, int basisSize, int ldV,
   ...
   double *prevhSvals, double machEps, double *rwork, int rworkSize,
   ...
   if (V == NULL) {
      ...
      if (primme->locking) {
         rworkSize += restart_locking_dprimme(&basisSize, NULL, NULL, nLocal,
               ...
               *numPrevRetained, NULL, NULL, 0.0, NULL, 0, NULL, primme);
      }
      else {
         rworkSize += restart_soft_locking_dprimme(&basisSize, NULL, NULL,
               ...
               numConverged, numConverged, *numPrevRetained, NULL, NULL, 0.0,
               NULL, 0, NULL, primme);
      }
   ...
   *numPrevRetained = max(0, min(min(
   ...
   Num_copy_matrix_dprimme(previousHVecs, basisSize, *numPrevRetained,
         ldpreviousHVecs, &hVecs[ldhVecs*restartSize], ldhVecs);

   ret = ortho_coefficient_vectors_dprimme(hVecs, basisSize, ldhVecs,
         restartSize, hU, ldhU, R, ldR, numPrevRetained, prevhSvals,
   ...
   if (!primme->locking) {
      ...
      ret = restart_soft_locking_dprimme(&restartSize, V, W, nLocal,
            ...
            *numPrevRetained, &indexOfPreviousVecs, hVecsPerm, machEps, rwork,
...
static int restart_soft_locking_dprimme(int *restartSize, double *V,
       ...
       int *hVecsPerm, double machEps, double *rwork, int rworkSize,
       int *iwork, primme_params *primme) {
...
static int restart_qr(double *V, int ldV, double *W, int ldW, double *H,
   ...
   if (*targetShiftIndex < 0 || primme->targetShifts[*targetShiftIndex]
      ...
      *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
      ...
      if (QtV) ret = update_projection_dprimme(Q, ldQ, V, ldV, QtV, ldQtV, nLocal, 0, restartSize,
   ...
   if (QtV) {
      Num_gemm_dprimme("N", "N", basisSize, restartSize, basisSize, tpone, QtV, ldQtV,
            hVecs, ldhVecs, tzero, rwork, basisSize);
      Num_copy_matrix_dprimme(rwork, basisSize, restartSize, basisSize, QtV, ldQtV);
   }
   ...
   ret = ortho_dprimme(hVecsRot, ldhVecsRot, R, ldR, 0, numArbitraryVecs-1,
         NULL, 0, 0, basisSize, primme->iseed, machEps, rwork, rworkSize, NULL);

   /* hU <- hU*q */
   Num_copy_matrix_dprimme(hU, basisSize, numArbitraryVecs, ldhU, rwork,
         basisSize);
   Num_gemm_dprimme("N", "N", basisSize, numArbitraryVecs, numArbitraryVecs,
   ...
   if (QtV) {
      Num_gemm_dprimme("C", "N", restartSize, restartSize, basisSize, tpone, hU, ldhU, QtV, ldQtV,
            tzero, rwork, restartSize);
      Num_copy_matrix_dprimme(rwork, restartSize, restartSize, restartSize, QtV, ldQtV);
   }
   ...
   ret = solve_H_dprimme(H, numArbitraryVecs, ldH, R, ldR, QtV, ldQtV,
...
static int ortho_coefficient_vectors_dprimme(double *hVecs, int basisSize,
   ...
   if (!hVecs) {
      ...
      for (i=ret=0; i<*numPrevRetained; i++) {
         ...
         Num_copy_matrix_dprimme(&hVecs[ldhVecs*(indexOfPreviousVecs+i)],
