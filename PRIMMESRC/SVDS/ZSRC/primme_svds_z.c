...
static Complex_Z* copy_last_params_from_svds(primme_svds_params *primme_svds, int stage,
   ...
   case primme_svds_op_AAt:
      ...
      Num_copy_Complex_Zmatrix(&svecs[primme_svds->mLocal*n], primme_svds->nLocal,
   ...
   case primme_svds_op_augmented:
      ...
      aux = (Complex_Z *)primme_calloc((primme_svds->mLocal+primme_svds->nLocal)*n,
         sizeof(Complex_Z), "aux");
      Num_zcopy_zprimme((primme_svds->mLocal+primme_svds->nLocal)*n,
         svecs, 1, aux, 1);
      Num_copy_Complex_Zmatrix(&aux[primme_svds->mLocal*n], primme_svds->nLocal,
         n, primme_svds->nLocal, svecs, primme_svds->mLocal+primme_svds->nLocal);
      Num_copy_Complex_Zmatrix(aux, primme_svds->mLocal, n, primme_svds->mLocal,
...
static void copy_last_params_to_svds(primme_svds_params *primme_svds, int stage,
   ...
   if (n0 > 0) {
      ...
      case primme_svds_op_AtA:
         ...
         Num_scalInv_Complex_Zmatrix(&svecs[primme_svds->mLocal*primme_svds->numOrthoConst],
      ...
      case primme_svds_op_augmented:
         ...
         Num_scal_zprimme(n0*(primme_svds->mLocal+primme_svds->nLocal), ztmp,
            svecs, 1);

         /* Shuffle svecs from [V; U] to [U V] */
         aux = (Complex_Z *)primme_calloc((primme_svds->mLocal+primme_svds->nLocal)*n0,
            sizeof(Complex_Z), "aux");
         Num_zcopy_zprimme((primme_svds->mLocal+primme_svds->nLocal)*n0,
            svecs, 1, aux, 1);
         Num_copy_Complex_Zmatrix(aux, primme_svds->nLocal, n0,
            primme_svds->mLocal+primme_svds->nLocal, &svecs[primme_svds->mLocal*n],
            primme_svds->nLocal);
         Num_copy_Complex_Zmatrix(&aux[primme_svds->nLocal], primme_svds->mLocal, n0,
   ...
   assert(primme_svds->realWork == (Complex_Z*)primme_svds->realWork + cut);
