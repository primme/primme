 ...
 *
 *    Calling dprimme_svds with all svals, svecs, resNorms set to NULL
...
int dprimme_svds(double *svals, double *svecs, double *resNorms, 
   ...
   /* Execute stage 1 */
   svecs0 = copy_last_params_from_svds(primme_svds, 0, NULL, svecs);
...
static double* copy_last_params_from_svds(primme_svds_params *primme_svds, int stage,
   ...
   case primme_svds_op_AAt:
      ...
      Num_copy_doublematrix(&svecs[primme_svds->mLocal*n], primme_svds->nLocal,
   ...
   case primme_svds_op_augmented:
      ...
      aux = (double *)primme_calloc((primme_svds->mLocal+primme_svds->nLocal)*n,
         sizeof(double), "aux");
      Num_dcopy_dprimme((primme_svds->mLocal+primme_svds->nLocal)*n,
         svecs, 1, aux, 1);
      Num_copy_doublematrix(&aux[primme_svds->mLocal*n], primme_svds->nLocal,
         n, primme_svds->nLocal, svecs, primme_svds->mLocal+primme_svds->nLocal);
      Num_copy_doublematrix(aux, primme_svds->mLocal, n, primme_svds->mLocal,
...
static int allocate_workspace_svds(primme_svds_params *primme_svds, int allocate) {
   ...
   if (primme_svds->method != primme_svds_op_none) {
      ...
      if ((primme.matrixMatvec == NULL || primme.matrixMatvec == matrixMatvecSVDS) &&
         ...
         realWorkSize += primme.maxBlockSize * sizeof(double) *
...
static void copy_last_params_to_svds(primme_svds_params *primme_svds, int stage,
   ...
   if (n0 > 0) {
      ...
      case primme_svds_op_AtA:
         ...
         Num_scalInv_doublematrix(&svecs[primme_svds->mLocal*primme_svds->numOrthoConst],
      ...
      case primme_svds_op_augmented:
         ...
         ztmp = sqrt(2.0L);
         Num_scal_dprimme(n0*(primme_svds->mLocal+primme_svds->nLocal), ztmp,
            svecs, 1);

         /* Shuffle svecs from [V; U] to [U V] */
         aux = (double *)primme_calloc((primme_svds->mLocal+primme_svds->nLocal)*n0,
            sizeof(double), "aux");
         Num_dcopy_dprimme((primme_svds->mLocal+primme_svds->nLocal)*n0,
            svecs, 1, aux, 1);
         Num_copy_doublematrix(aux, primme_svds->nLocal, n0,
            primme_svds->mLocal+primme_svds->nLocal, &svecs[primme_svds->mLocal*n],
            primme_svds->nLocal);
         Num_copy_doublematrix(&aux[primme_svds->nLocal], primme_svds->mLocal, n0,
   ...
   assert(primme_svds->realWork == (double*)primme_svds->realWork + cut);
   ...
   if (stage == 1 || primme_svds->method0 == primme_svds_op_none) {
      switch(method) {
      case primme_svds_op_AtA:
      case primme_svds_op_AAt:
         for (i=0; i<primme_svds->initSize; i++) {
...
static void Num_copy_doublematrix(double *x, int m, int n, int ldx, double *y, int ldy) {
   int i,j;
...
static void Num_scalInv_doublematrix(double *x, int m, int n, int ldx, double *factors) {
   int i;
   double ztmp;
