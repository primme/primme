...
int dprimme_svds(double *svals, double *svecs, double *resNorms, 
   ...
   if (primme_svds->svdsMethod == primme_svds_normalequations ||
      ...
      if (primme_svds->svdsMethod == primme_svds_normalequations){ 
          if (primme_svds->m >= primme_svds->n){
              ...
              Num_dcopy_dprimme(primme_svds->n*
          ...
          if (primme_svds->m >= primme_svds->n){
              ...
              /* Copy initial guesses to proper place of svecs */
              Num_dcopy_dprimme(primme_svds->m*
                    primme_svds->initSize, svecs, 1, &svecs[
      ...
      if (primme_svds->svdsMethod == primme_svds_normalequations) {
          if (primme_svds->m >= primme_svds->n){
              for (i=0;i<primme_svds->numSvals;i++){
                  ...
                  ztmp = 1.0L/svals[i];
                  Num_scal_dprimme(primme_svds->m, ztmp, 
              ...
              for (i=0;i<primme_svds->numSvals;i++){
                  ...
                  ztmp = 1.0L/svals[i];
                  Num_scal_dprimme(primme_svds->n, ztmp, 
      ...
      [v1 u1, v2 u2,...,v3 u3, vk uk].  */
          ...
          if (primme_svds->m >= primme_svds->n){
              for (i=0;i<primme_svds->numSvals;i++){
                  svals[i] = sqrt(fabs(svals[i]));
                  Num_dcopy_dprimme(primme_svds->n, &svecs[
                  ...
                  ztmp = 1.0L/svals[i];
                  Num_scal_dprimme(primme_svds->m, ztmp, &svecs[
              ...
              for (i=0;i<primme_svds->numSvals;i++){
                  ...
                  ztmp = 1.0L/svals[i];
                  Num_scal_dprimme(primme_svds->n, ztmp, &svecs[
                    i*(primme_svds->m+primme_svds->n)], 1);
                  Num_dcopy_dprimme(primme_svds->m, &svecs[
                        primme_svds->numSvals*primme_svds->n + 
   ...
   if (primme_svds->svdsMethod == primme_svds_augmented || 
      ...
      /* -------------------------------------------------------------*/
      ztmp = sqrt(2.0L);
      Num_scal_dprimme((primme_svds->m + primme_svds->n)*
      ...
      /* Suppose returned eigenvectors are [v1 u1 v2 u2 ... vk uk],
         ...
         svecs memory for V.  */
      for (i=0;i<primme_svds->numSvals;i++){
          Num_dcopy_dprimme(primme_svds->n, &svecs[i*
                (primme_svds->n + primme_svds->m)], 1, &realWork[i*
                primme_svds->n], 1); 
          Num_dcopy_dprimme(primme_svds->m, &svecs[i*
                (primme_svds->m+primme_svds->n) + primme_svds->n], 1, 
                &svecs[i*(primme_svds->n+ primme_svds->m)], 1); 
      }
      Num_dcopy_dprimme(primme_svds->n*primme_svds->numSvals, 
