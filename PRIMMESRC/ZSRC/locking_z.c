...
int restart_locking_zprimme(int *restartSize, Complex_Z *V, Complex_Z *W, 
   ...
   *R = &W[*restartSize*ldV];
   ...
   if (1 /* Put zero to disable the new feature */) {
      ...
      Num_compact_vecs_zprimme(&V[left*ldV], nLocal, failed, ldV,
            ifailed, &V[left*ldV], ldV, 0);
      Num_compact_vecs_zprimme(&W[left*ldV], nLocal, failed, ldV,
            ifailed, &W[left*ldV], ldV, 0);

      /* Copy X and R after the failed vectors */

      Num_copy_matrix_zprimme(*X, nLocal, sizeBlockNorms, ldV,
            &V[(left+failed)*ldV], ldV);
      Num_copy_matrix_zprimme(*R, nLocal, sizeBlockNorms, ldV,
   ...
   Num_compact_vecs_zprimme(&hVecs[left*ldhVecs], basisSize, failed,
