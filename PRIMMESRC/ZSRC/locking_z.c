...
int restart_locking_zprimme(int *restartSize, Complex_Z *V, Complex_Z *W,
   ...
   int ldhVecs, int *hVecsperm, double *hVals, int *flags, int *iev, int *ievSize,
   ...
   int *numPrevRetained, int ldpreviousHVecs, int *indexOfPreviousVecs, int *Vperm,
   ...
   if (V == NULL) {
      ...
      return max(max(max(max(
            /* for permute_vecs */
            basisSize,
            Num_compact_res_i_zprimme(nLocal, NULL, NULL, basisSize, NULL, 0,
            ...
            ortho_zprimme(NULL, 0, NULL, 0, 0,
               *numPrevRetained-1, NULL, 0, *restartSize, 0,
               NULL, 0, NULL, 0, NULL)),
            Num_update_VWXR_z(NULL, NULL, 0, basisSize, 0, NULL,
               *restartSize, 0, NULL,
   ...
   *indexOfPreviousVecs = *restartSize;
   ...
   for (i=j=0; i<basisSize; i++) {
      if (flags[i] == UNCONVERGED) {
         hVecsperm[j < *restartSize+*numPrevRetained ? j : *numConverged-*numLocked+j] = i;
         j++;
      }
   }
   ...
   permute_vecs_z(hVecs, basisSize, basisSize, ldhVecs, hVecsperm, rwork, iwork);
   ...
   *restartSize += numPacked + *numPrevRetained;
   ...
   permute_vecs_i(flags, basisSize, hVecsperm, iwork);
   ...
   /* ----------------------------------------------------------------------------- */
   /* Pack hVals, hVecs, blockNorms and hVecsperm for the unconverged pairs.        */
   ...
   if (1 /* Put zero to disable the new feature */) {
      ...
      for (i=j=k=0; i<*indexOfPreviousVecs || j<failed; k++) {
         if (i < *indexOfPreviousVecs && (j >= failed || hVecsperm[i] < hVecsperm[left+ifailed[j]])) {
      ...
      Num_compact_res_i_zprimme(nLocal, &hVals[left], &V[left*ldV], failed, ifailed, 
            ldV, &W[left*ldV], ldV,
      ...
      Num_compact_vecs_z(&V[left*ldV], nLocal, failed, ldV, ifailed, &V[left*ldV],
            ldV, 0);
      Num_compact_vecs_z(&W[left*ldV], nLocal, failed, ldV, ifailed, &W[left*ldV],
            ldV, 0);
   ...
   Num_compact_vecs_z(&hVecs[left*ldhVecs], basisSize, failed, ldhVecs, ifailed,
