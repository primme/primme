...
int main_iter_zprimme(double *evals, int *perm, SCALAR *evecs, 
   ...
   if (primme->n == 1) {
      evecs[0] = 1.0;
      CHKERR(matrixMatvec_zprimme(&evecs[0], primme->nLocal, primme->nLocal,
   ...
   CHKERR(init_basis_zprimme(V, primme->nLocal, primme->nLocal, W,
   ...
   while (!converged &&
      ...
      targetShiftIndex = 0;
      if (Q) CHKERR(update_Q_zprimme(V, primme->nLocal, primme->nLocal, W,
      ...
      while (numConverged < primme->numEvals &&
         ...
         while (basisSize < primme->maxBasisSize &&
            ...
            if (availableBlockSize > 0) {
               prepare_candidates_zprimme(V, W, primme->nLocal, H,
            ...
            if (blockSize == 0) {
               ...
               CHKERR(solve_correction_zprimme(V, W, evecs, evecsHat, UDU, 
            ...
            CHKERR(ortho_zprimme(V, primme->nLocal, NULL, 0, basisSize, 
            ...
            CHKERR(matrixMatvec_zprimme(V, primme->nLocal, primme->nLocal,
                     W, primme->nLocal, basisSize, blockSize, primme), -1);

            if (Q) CHKERR(update_Q_zprimme(V, primme->nLocal,
            ...
            if (basisSize+blockSize >= primme->maxBasisSize) {
               CHKERR(retain_previous_coefficients_zprimme(hVecs,
         ...
         if (targetShiftIndex >= 0) {
            ...
            permute_vecs_zprimme(hVecs, basisSize, basisSize, basisSize, iwork, rwork, iwork+basisSize);
      ...
      if (primme->locking) {
         ...
         if (restartLimitReached || converged || wholeSpace) {
            ...
            /* ------------------------------------------------------------ */
            CHKERR(ortho_zprimme(V, primme->nLocal, NULL, 0, 0,
...
int prepare_candidates_zprimme(SCALAR *V, SCALAR *W, PRIMME_INT nLocal,
   ...
   if (V == NULL) {
      ...
      lrw = max(lrw,
            (size_t)Num_update_VWXR_zprimme(NULL, NULL, nLocal, basisSize,
               ...
               NULL, 0, 0,
               NULL, 0, primme));
      CHKERR(prepare_vecs_zprimme(basisSize, 0, maxBlockSize, NULL, 0,
   ...
   *recentlyConverged = 0;
   while (1) {
      ...
      CHKERR(check_convergence_zprimme(X?&X[(*blockSize)*ldV]:NULL, nLocal,
      ...
      for (blki=*blockSize, i=0; i < blockNormsSize && *blockSize < maxBlockSize; i++, blki++) {
         ...
         if ((primme->target == primme_closest_leq
            ...
            *smallestResNorm = min(*smallestResNorm, blockNorms[blki]);
            ...
            if (X) Num_copy_matrix_zprimme(&X[blki*ldV], nLocal, 1, ldV,
                  &X[(*blockSize)*ldV], ldV);
            if (R) Num_copy_matrix_zprimme(&R[blki*ldV], nLocal, 1, ldV,
                  &R[(*blockSize)*ldV], ldV);
            (*blockSize)++;
         }
      ...
      if (blki > 0)
         ...
         i = 0;
      blki = *blockSize;
      prepare_vecs_zprimme(basisSize, i, maxBlockSize-blki, H, ldH, hVals,
      ...
      CHKERR(Num_update_VWXR_zprimme(V, W, nLocal, basisSize, ldV,
...
static int verify_norms(SCALAR *V, SCALAR *W, double *hVals, int basisSize,
   ...
   CHKERR(check_convergence_zprimme(V, primme->nLocal, primme->nLocal,
