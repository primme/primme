...
static int apply_skew_projector(SCALAR *Q, SCALAR *Qhat, SCALAR *UDU, 
   ...
   if (numCols > 0) {    /* there is a projector to be applied */
      ...
      if (numCols == 1) {
         ...
         CHKERR(globalSum_zprimme(workSpace, overlaps, numCols, primme),
               -1);
         ...
         if (UDU != NULL) {
            ...
            CHKERR(UDUSolve_zprimme(UDU, ipivot, numCols, overlaps,
                     workSpace), -1);
...
static int apply_projected_matrix(SCALAR *v, double shift, SCALAR *Q, 
   ...
   CHKERR(matrixMatvec_zprimme(v, primme->nLocal, primme->nLocal, result,
...
static int apply_projector(SCALAR *Q, int numCols, SCALAR *v, 
   ...
   Num_gemv_zprimme("C", primme->nLocal, numCols, 1.0, Q, primme->nLocal,
      v, 1, 0.0, workSpace, 1);
   CHKERR(globalSum_zprimme(workSpace, overlaps, numCols, primme), -1);
...
static int dist_dot(SCALAR *x, int incx,
   ...
   CHKERR(globalSum_zprimme(&temp, result, 1, primme), -1);
...
static int dist_dot_real(SCALAR *x, int incx,
   ...
   temp = Num_dot_zprimme(primme->nLocal, x, incx, y, incy);
   CHKERR(globalSum_zprimme(&temp, &product, 1, primme), -1);
