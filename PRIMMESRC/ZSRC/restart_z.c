...
static int restart_soft_locking_zprimme(int *restartSize, SCALAR *V,
...
static int restart_projection_zprimme(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
...
static int dtr_zprimme(int numLocked, SCALAR *hVecs, double *hVals, int *flags, 
  int basisSize, int numFree, int *iev, SCALAR *rwork, primme_params *primme);

static int ortho_coefficient_vectors_zprimme(SCALAR *hVecs, int basisSize,
...
int restart_zprimme(SCALAR *V, SCALAR *W, PRIMME_INT nLocal, int basisSize,
   ...
   if (V == NULL) {
      iworkSize0 = 0;
      CHKERR(ortho_coefficient_vectors_zprimme(NULL, basisSize, 0,
            basisSize, NULL, 0, NULL, 0, numPrevRetained, 0.0, NULL, rworkSize,
            primme), -1);

      if (primme->locking) {
         CHKERR(restart_locking_zprimme(&basisSize, NULL, NULL, nLocal,
         ...
         CHKERR(restart_soft_locking_zprimme(&basisSize, NULL, NULL,
      ...
      CHKERR(restart_projection_zprimme(NULL, 0, NULL, 0, NULL, 0, NULL, 0, 0,
   ...
   *numPrevRetained = max(0, min(min(
   ...
   CHKERR(ortho_coefficient_vectors_zprimme(hVecs, basisSize, ldhVecs,
   ...
   if (!primme->locking) {
      SCALAR *X, *Res;
      CHKERR(restart_soft_locking_zprimme(&restartSize, V, W, nLocal,
      ...
      SCALAR *X, *Res;
      CHKERR(restart_locking_zprimme(&restartSize, V, W, nLocal, basisSize,
   ...
   *reset = 0;
   ...
   CHKERR(restart_projection_zprimme(V, ldV, W, ldV, H, ldH, Q, ldQ,
   ...
   if (*numConverged >= primme->numEvals && !primme->locking) {
      permute_vecs_zprimme(V, nLocal, restartSize, ldV, hVecsPerm, rwork,
            iwork0);
   }
...
static int restart_soft_locking_zprimme(int *restartSize, SCALAR *V,
   ...
   if (V == NULL) {
      ...
      *rworkSize = max(*rworkSize, (size_t)basisSize); /* permute_vecs for hVecs */
      CHKERR(Num_reset_update_VWXR_zprimme(NULL, NULL, nLocal, basisSize, 0, &t,
            *restartSize, 0, NULL,
            &t, 0, *restartSize, 0,
   ...
   *R = &W[*restartSize*ldV];
   ...
   if (!wholeSpace) {
      ...
      CHKERR(check_convergence_zprimme(V, nLocal, ldV, NULL, 0, NULL, 0, 0,
      ...
      *numConverged = 0;
      ...
      permute_vecs_dprimme(hVals, 1, *restartSize, 1, hVecsPerm, (double*)rwork,
            iwork);
      permute_vecs_zprimme(hVecs, basisSize, *restartSize, ldhVecs,
...
int Num_reset_update_VWXR_zprimme(SCALAR *V, SCALAR *W, PRIMME_INT mV,
   ...
   if (V == NULL) {
      *lrwork = max(*lrwork,
            (size_t)Num_update_VWXR_zprimme(
   ...
   /* Quick exit */
   if (reset == 0) {
      CHKERR(Num_update_VWXR_zprimme(
               V, W, mV, nV, ldV, h, nh, ldh, hVals,
               X0, nX0b, nX0e, ldX0,
               X1, nX1b, nX1e, ldX1,
   ...
   if (reset > 1) {
      CHKERR(ortho_zprimme(evecs, ldevecs, NULL, 0, evecsSize, 
               evecsSize+nX2e-nX2b-1, NULL, 0, 0, mV, primme->iseed, 
               machEps, rwork, lrwork, primme), -1);
      CHKERR(ortho_zprimme(X0, ldX0, NULL, 0, 0, nX2b-nX0b-1, evecs,
      ...
      CHKERR(ortho_zprimme(X0, ldX0, NULL, 0, nX2e-nX0b, nX0e-nX0b-1,
   ...
   CHKERR(matrixMatvec_zprimme(X0, mV, ldX0, Wo, ldWo, 0, nWoe-nWob,
            primme), -1);
   ...
   for (j=nRb; j<nRe; j++) {
      ...
      if (Rnorms) {
         Rnorms[j-nRb] = REAL_PART(Num_dot_zprimme(mV, &R[ldR*(j-nRb)], 1,
                  &R[ldR*(j-nRb)], 1));
      }
   }
   ...
   if (rnorms) for (j=nrb; j<nre; j++) {
      ...
      for (i=0; i < mV; i+=m, m=min(m,mV-i)) {
         ...
         rnorms[j-nrb] +=
            REAL_PART(Num_dot_zprimme(m, rwork, 1, rwork, 1));
      }
   }
...
static int restart_projection_zprimme(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
   ...
   if (evecsHat) {
      ...
      /* Return memory requirement */
      if (H == NULL) {
         CHKERR(update_projection_zprimme(NULL, 0, NULL, 0, NULL, 0, nLocal,
                  *evecsSize, basisSize, NULL, rworkSize, 1/*symmetric*/,
                  primme), -1);
         CHKERR(UDUDecompose_zprimme(NULL, 0, NULL, 0, NULL, *evecsSize,
      ...
      CHKERR(update_projection_zprimme(evecs, primme->nLocal, evecsHat,
      ...
      *evecsSize = numConverged;
      ...
      CHKERR(UDUDecompose_zprimme(M, ldM, UDU, ldUDU, ipivot,
...
static int restart_RR(SCALAR *H, int ldH, SCALAR *hVecs, int ldhVecs,
   ...
   if (H == NULL) {
      CHKERR(compute_submatrix_zprimme(NULL, numPrevRetained, 0, NULL,
               basisSize, 0, NULL, 0, NULL, rworkSize), -1);
      CHKERR(solve_H_zprimme(NULL, numPrevRetained, 0, NULL, 0, NULL, 0,
   ...
   CHKERR(compute_submatrix_zprimme(&hVecs[ldhVecs*indexOfPreviousVecs],
...
static int restart_refined(SCALAR *V, PRIMME_INT ldV, SCALAR *W, PRIMME_INT ldW,
   ...
   if (V == NULL) {
      CHKERR(compute_submatrix_zprimme(NULL, basisSize, 0, NULL, basisSize,
               0, NULL, 0, NULL, rworkSize), -1);
      CHKERR(update_Q_zprimme(NULL, nLocal, 0, NULL, 0, NULL, 0, NULL, 0,
               0.0, 0, basisSize, NULL, rworkSize, 0.0, primme), -1);
      CHKERR(update_projection_zprimme(NULL, 0, NULL, 0, NULL, 0, nLocal, 0, basisSize,
      ...
      *rworkSize = max(*rworkSize,
            (size_t)Num_update_VWXR_zprimme(NULL, NULL, nLocal, basisSize,
               ...
               NULL, 0, 0,
               NULL, 0, primme));
      CHKERR(solve_H_zprimme(NULL, basisSize, 0, NULL, 0, NULL, 0, NULL, 0,
   ...
   if (H) compute_submatrix_zprimme(hVecs, restartSize, ldhVecs, H,
   ...
   if (*targetShiftIndex < 0 || primme->targetShifts[*targetShiftIndex]
      ...
      *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
      ...
      CHKERR(update_Q_zprimme(V, nLocal, ldV, W, ldW, Q, ldQ, R, ldR,
               primme->targetShifts[*targetShiftIndex], 0,
               restartSize, rwork, rworkSize, machEps, primme), -1);

      CHKERR(solve_H_zprimme(H, restartSize, ldH, R, ldR, NULL, 0, hU,
   ...
   Num_gemm_zprimme("N", "N", basisSize, numPrevRetained, basisSize, 1.0,
   ...
   Num_zero_matrix_zprimme(hVecsRot0, mhVecsRot0, nRegular, mhVecsRot0);
   Num_copy_matrix_columns_zprimme(hVecsRot, *numArbitraryVecs,
   ...
   /* R = zeros() */
   Num_zero_matrix_zprimme(R, primme->maxBasisSize, primme->maxBasisSize,
         ldR);
   
   /* [hVecsRot0, R] = ortho(hVecsRot0) */
   CHKERR(ortho_zprimme(hVecsRot0, mhVecsRot0, R, ldR, 0, nRegular-1, NULL,
         0, 0, mhVecsRot0, primme->iseed, machEps, rwork0, &rworkSize0, NULL),
         -1);

   /* hU = hU * hVecsRot0 */
   Num_gemm_zprimme("N", "N", basisSize, nRegular, mhVecsRot0, 1.0, hU,
         ldhU, hVecsRot0, mhVecsRot0, 0.0, rwork0, basisSize);
   Num_copy_matrix_zprimme(rwork0, basisSize, nRegular, basisSize, hU,
         basisSize);

   /* hU = [hU RPrevhVecs] */
   Num_copy_matrix_zprimme(RPrevhVecs, basisSize, numPrevRetained,
         basisSize, &hU[basisSize*nRegular], basisSize);

   /* [hU, R] = ortho(hU, nRegular:nRegular+numPrevRetained-1) */
   CHKERR(ortho_zprimme(hU, basisSize, R, ldR, nRegular,
   ...
   /* to the retained vectors should be zero.                                 */
    ...
    if (*numArbitraryVecs <= indexOfPreviousVecsBeforeRestart) {
      ...
      Num_zero_matrix_zprimme(&R[ldR*nRegular], nRegular, numPrevRetained,
            ldR);
   }
   ...
   CHKERR(Num_update_VWXR_zprimme(Q, NULL, nLocal, basisSize, ldQ, hU,
            restartSize,
            basisSize, NULL,
            Q, 0, restartSize, ldQ,
            NULL, 0, 0, 0,
   ...
   CHKERR(solve_H_zprimme(H, restartSize, ldH, R, ldR, NULL, 0, hU, newldhU,
   ...
   /* R = R(:, invhVecsPerm) */
   permute_vecs_zprimme(R, restartSize, restartSize, ldR, invhVecsPerm,
         rwork, iwork0);
   ...
   Num_zero_matrix_zprimme(hVecsRot, primme->maxBasisSize, primme->maxBasisSize,
         ldhVecsRot);
   for (j=0; j < *numArbitraryVecs; j++) {
      for (i=0; i<restartSize; i++) {
   ...
   Num_zero_matrix_zprimme(hVecs, restartSize, *numArbitraryVecs, newldhVecs);
   for (j=0; j < *numArbitraryVecs; j++) {
      hVecs[newldhVecs*j+hVecsPerm[j]] = 1.0;
   }
...
static int restart_harmonic(SCALAR *V, PRIMME_INT ldV, SCALAR *W,
   ...
   CHKERR(compute_submatrix_zprimme(hVecs, restartSize, ldhVecs, H,
   ...
   *targetShiftIndex = min(primme->numTargetShifts-1, numConverged);
   ...
   CHKERR(update_projection_zprimme(Q, ldQ, V, ldV, QtV, ldQtV, nLocal, 0,
   ...
   CHKERR(solve_H_zprimme(H, restartSize, ldH, R, ldR, QtV, ldQtV, hU,
            newldhU, hVecs, newldhVecs, hVals, hSVals, numConverged, machEps,
            rworkSize, rwork, iworkSize, iwork, primme), -1);
  ...
  int basisSize, int numFree, int *iev, SCALAR *rwork, primme_params *primme)
{
   ...
   Num_copy_zprimme(i*basisSize, &hVecs[basisSize*lOpt], 1, rwork, 1);
   Num_copy_zprimme(rOpt*basisSize, &hVecs[basisSize*(basisSize-rOpt)], 1,
      &hVecs[basisSize*lOpt], 1);
   Num_copy_zprimme(i*basisSize, rwork, 1, &hVecs[basisSize*restartSize], 1);
...
static int ortho_coefficient_vectors_zprimme(SCALAR *hVecs, int basisSize,
   ...
   if (primme->projectionParams.projection == primme_proj_harmonic) {
      ...
      CHKERR(ortho_zprimme(hVecs?&hVecs[ldhVecs*indexOfPreviousVecs]:NULL,
      ...
      if (hVecs) Num_trsm_zprimme("L", "U", "N", "N", basisSize,
   ...
   CHKERR(ortho_zprimme(hVecs, ldhVecs, !outR ? NULL :
...
int retain_previous_coefficients_zprimme(SCALAR *hVecs, int ldhVecs,
   ...
   *numPrevRetained = i;
   ...
   if (primme->projectionParams.projection == primme_proj_RR
      ...
      Num_zero_matrix_zprimme(previousHVecs+basisSize, mprevious-basisSize,
