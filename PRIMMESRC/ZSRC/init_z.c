...
int init_basis_zprimme(Complex_Z *V, Complex_Z *W, Complex_Z *evecs, 
   ...
   if (!primme->locking) {
      ...
      /* the user                                                     */
        ...
        /* lingfei: primme_svds. change ortho function for returning Q and R */
         ret = ortho_zprimme(V, NULL, primme->nLocal, 0, primme->initSize-1, 
   ...
   /*-----------------------------------------------------------------------*/
      ...
      *nextGuess = primme->numOrthoConst;
      ...
      if (primme->initSize > 0) {
        ...
        /* lingfei: primme_svds. change ortho function for returning Q and R */
         ret = ortho_zprimme(V, NULL, primme->nLocal, 0, currentSize-1, evecs,
   ...
   if (*basisSize < primme->maxBlockSize) {
      Num_larnv_zprimme(2, primme->iseed,primme->nLocal*(primme->maxBlockSize-*basisSize),&V[primme->nLocal*(*basisSize)]);
      ret = ortho_zprimme(V, NULL, primme->nLocal, *basisSize, primme->maxBlockSize, evecs, 
      ...
      update_W_zprimme(V, W, *basisSize, primme->maxBlockSize, primme);
...
static int init_block_krylov(Complex_Z *V, Complex_Z *W, int dv1, int dv2, 
   ...
   if (numNewVectors <= primme->maxBlockSize) {
     ...
     /* lingfei: primme_svds. change ortho function for returning Q and R */
      ret = ortho_zprimme(V, NULL, primme->nLocal, dv1, dv1, locked, 
      ...
      for (i = dv1; i < dv2; i++) {
         ...
         Num_zcopy_zprimme(primme->nLocal, &V[primme->nLocal*(i+1)], 1,
            &W[primme->nLocal*i], 1);
        /* lingfei: primme_svds. change ortho function for returning Q and R */
         ret = ortho_zprimme(V, NULL, primme->nLocal, i+1, i+1, locked, 
            primme->nLocal, numLocked, primme->nLocal, primme->iseed, machEps,
            rwork, rworkSize, primme);
