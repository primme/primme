...
int init_basis_zprimme(SCALAR *V, PRIMME_INT nLocal, PRIMME_INT ldV,
   ...
   if (primme->numOrthoConst > 0) {
      CHKERR(ortho_zprimme(evecs, ldevecs, NULL, 0, 0, 
        primme->numOrthoConst - 1, NULL, 0, 0, nLocal, 
      ...
      if (UDU != NULL) {
         ...
         CHKERR(update_projection_zprimme(evecs, ldevecs, evecsHat,
   ...
   *basisSize = initSize + random;
   ...
   CHKERR(ortho_zprimme(V, ldV, NULL, 0, 0, *basisSize-1, 
         evecs, ldevecs, primme->numOrthoConst, nLocal, 
         primme->iseed, machEps, rwork, rworkSize, primme), -1)

   CHKERR(matrixMatvec_zprimme(V, nLocal, ldV, W, ldW, 0, *basisSize,
            primme), -1);
   ...
   if (primme->dynamicMethodSwitch && *basisSize < primme->maxBasisSize) {
      *timeForMV = primme_wTimer(0);
      CHKERR(matrixMatvec_zprimme(V, nLocal, ldV, &W[ldW*(*basisSize)],
               ldV, 0, 1, primme), -1);
...
static int init_block_krylov(SCALAR *V, PRIMME_INT nLocal, PRIMME_INT ldV,
   ...
   CHKERR(ortho_zprimme(V, ldV, NULL, 0, dv1, 
   ...
   for (i = dv1+blockSize; i <= dv2; i++) {
      CHKERR(matrixMatvec_zprimme(&V[ldV*(i-blockSize)], nLocal, ldV,
               &V[ldV*i], ldV, 0, 1, primme), -1);

      Num_copy_zprimme(nLocal, &V[ldV*i], 1,
         &W[ldW*(i-blockSize)], 1);

      CHKERR(ortho_zprimme(V, ldV, NULL, 0, i, i, locked, 
               ldlocked, numLocked, nLocal, primme->iseed, machEps,
               rwork, rworkSize, primme), -1);
   }

   CHKERR(matrixMatvec_zprimme(V, nLocal, ldV, W, ldW, dv2-blockSize+1,
            blockSize, primme), -1);
