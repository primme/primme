...
int solve_H_zprimme(Complex_Z *H, int basisSize, int ldH, Complex_Z *R, int ldR,
   ...
   case primme_proj_harmonic:
      ret = solve_H_Harm_zprimme(H, ldH, QtV, ldQtV, R, ldR, hVecs, ldhVecs, hU,
...
static int solve_H_Harm_zprimme(Complex_Z *H, int ldH, Complex_Z *QtV, int ldQtV,
   ...
   /* Return memory requirements */
   if (QtV == NULL) {
      return solve_H_RR_zprimme(QtV, ldQtV, hVecs, ldhVecs, hVals, basisSize,
...
int prepare_vecs_zprimme(int basisSize, int i0, int blockSize,
   ...
   if (H == NULL) {
      return basisSize*basisSize + /* aH */
         max(
               compute_submatrix_zprimme(NULL, basisSize, 0, NULL, basisSize, 0,
                  NULL, 0, NULL, 0),
               solve_H_RR_zprimme(NULL, 0, NULL, 0, NULL, basisSize, 0, 0, NULL,
                  NULL, primme));
   }
   ...
   for (candidates=0, i=min(*arbitraryVecs,basisSize), j=i0;
      ...
      if (i-j > 1 && (someCandidate || RRForAll)) {
         ...
         /* Zero hVecsRot(:,j:i-1) */
         Num_zero_matrix_zprimme(&hVecsRot[ldhVecsRot*j],
         ...
         /* Compute and sort eigendecomposition aH*ahVecs = ahVecs*diag(hVals(j:i-1)) */
         ret = solve_H_RR_zprimme(aH, aBasisSize, ahVecs, ldhVecsRot,
   ...
   permute_vecs_zprimme(&hVecs[ldhVecs*i0], basisSize, i-i0, ldhVecs, perm, rwork, iwork);
   permute_vecs_zprimme(&hVecsRot[ldhVecsRot*i0], basisSize, i-i0, ldhVecsRot, perm, rwork, iwork);
