...
c-----------------------------------------------------------------------
       subroutine zpgmres(n, im, rhs, sol, vv, eps, maxits, iout,
...
c       implicit real*8 (a-h,o-z)
       ...
       real*8 eps
       complex*16 vv(n,*), rhs(n), sol(n), aa(*), alu(*)
c----------------------------------------------------------------------*
c                                                                      *
...
c BLAS1  routines.                                                     *
c----------------------------------------------------------------------*
       real*8 epsmac,eps1,  ro,gam, r1
       integer n1,its,j,i,i1,k,k1,ii,jj,kmax
       parameter (kmax=50)
       complex*16 hh(kmax+1,kmax), c(kmax), s(kmax), rs(kmax+1),t
       complex*16 zdotc
       external zdotc
c-------------------------------------------------------------
...
c-------------- compute initial residual vector --------------
       call zamux (n, sol, vv, aa, ja, ia)
       do 21 j=1,n
          vv(j,1) = rhs(j) - vv(j,1)
 21    continue
c-------------------------------------------------------------
 20    ro = dsqrt(dreal(zdotc(n,vv,1,vv,1))) 
...
4      i=i+1
       its = its + 1
       i1 = i + 1
       call zlusol(n, vv(1,i), rhs, alu, jlu, ju)
       call zamux(n, rhs, vv(1,i1), aa, ja, ia)
c-----------------------------------------
c     modified gram - schmidt...
c-----------------------------------------
       do 55 j=1, i
          t = zdotc(n, vv(1,i1),1,vv(1,j),1)
          hh(j,i) = t
          call zaxpy(n, -t, vv(1,j), 1, vv(1,i1), 1)
 55    continue

       r1 = dsqrt(dreal(zdotc(n, vv(1,i1), 1, vv(1,i1), 1)))
       hh(i1,i) = r1
...
c--------perfrom previous transformations  on i-th column of h
       do 66 k=2,i
          k1 = k-1
          t = hh(k1,i)
          hh(k1,i) = c(k1)*t + conjg(s(k1))*hh(k,i)
          hh(k,i) = -s(k1)*t + c(k1)*hh(k,i)
 66    continue
121    gam=dsqrt(dreal(hh(i,i)*conjg(hh(i,i))+hh(i1,i)*conjg(hh(i1,i))))
c
...
c
       c(i) = abs(hh(i,i))/gam
       s(i) = c(i)*hh(i1,i)/hh(i,i)
       rs(i1) = -s(i)*rs(i)
       rs(i) =  c(i)*rs(i)
c
c     detrermine residual norm and test for convergence-
c
       hh(i,i) = c(i)*hh(i,i) + conjg(s(i))*hh(i1,i)
       ro = abs(rs(i1))
...
c
       call zlusol (n, rhs, rhs, alu, jlu, ju)
       do 17 k=1, n
          sol(k) = sol(k) + rhs(k)
 17    continue
c
...
c
 ...
 24    continue
       do 25  j=1,i1
          t = rs(j)
          if (j .eq. 1)  t = t-(1.0d0,0.d0)
          call zaxpy (n, t, vv(1,j), 1,  vv, 1)
 25    continue
